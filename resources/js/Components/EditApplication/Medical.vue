...
const form = useForm({
    ...
    medical_details: props.application.medical_details.map(m => ({
        id: m.id,
        name: m.name || '',
        gender: m.gender || '',
        contact: m.contact || '',
...
const removeMedical = (index) => {
    if (index === 0) return;  // Prevent removing the first medical_detail
    form.medical_details.splice(index, 1)
}
...
const handleOpenDocument = (filePath) => {
    ...
    if (typeof filePath === 'string') {
        ...
        // For new files not yet uploaded, you can't open them directly
