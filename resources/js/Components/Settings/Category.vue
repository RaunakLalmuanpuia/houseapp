...
<template>
    <section class="flex-grow space-y-8 mr-5">
        <!-- Add Room Type form -->
        <div class="border border-gray-200 rounded-xl p-6 max-w-4xl" aria-label="Add Category">
            <h3 class="font-bold text-lg mb-6">Add Reason/Category</h3>
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-6 items-end">
                <!-- Input: spans 2 columns -->
                <div class="sm:col-span-2">
                    <label for="roomTypeName" class="block text-sm font-normal mb-1 text-black">Reason/Category Name</label>
                    <input
                        v-model="form.name"
                        id="roomTypeName"
                        name="roomTypeName"
                        type="text"
                        placeholder="Name of of Category"
        ...
        <section class="max-w-4xl space-y-4">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <h3 class="font-bold text-lg mb-3 sm:mb-0">List of Category</h3>
            ...
            <table class="w-full border-collapse text-sm relative">
                ...
                <tr class="bg-gray-700 text-white text-left">
                    <th class="py-2 px-3 font-semibold w-[10%]">SL.NO</th>
                    <th class="py-2 px-3 font-semibold w-[40%]">Reason/Category</th>
                    <th class="py-2 px-3 font-semibold w-[10%] text-center">ACTION</th>
                </tr>
                </thead>
                <tbody>
                <tr
                            ...
                            >
                                ...
                                >
                                    <g filter="url(#filter0_d_534_2390)">
                                        <path
                                            d="M11.4098 7.2998H11.3998M11.3098 11.9998H11.2998M11.4098 16.6998H11.3998"
                                            stroke="black"
                                            stroke-width="2.6"
                                    ...
                                    <defs>
                                        ...
                                        >
                                            ...
                                            <feBlend
                                                ...
                                                result="effect1_dropShadow_534_2390"
                                            />
                                            <feBlend
                                                mode="normal"
                                                in="SourceGraphic"
                                                in2="effect1_dropShadow_534_2390"
                                                result="shape"
                                            />
                                        </filter>
                                    </defs>
    ...
    <div v-if="showDeleteDialog"
        ...
        <div class="bg-white rounded-xl max-w-sm w-full p-6">
            <h2 class="font-semibold text-lg mb-4">Delete Reason/Category</h2>
            <p class="mb-8 text-base font-normal text-gray-900">I am sure, that i want to delete this Reason/Category</p>
...
const selectedCategory = ref({}) // optional, used for display if needed
...
function handleSearch(val) {
    getCategory({ pagination: pagination.value, filter: val });
}
function clearSearch() {
    filter.value = ''; // Clear the search value
    // Reset the pagination if needed (e.g., set page to 1 after clearing search)
    pagination.value.page = 1;
    getCategory({ pagination: pagination.value, filter: '' }); // Clear the filter in the API request
}
function getCategory(props) {
    const { page, rowsPerPage, sortBy, descending } = props.pagination;
    const filter = props.filter;

    axios.get(route('admin.rate-category.index'), {
        params: {
            filter,
            page,
            rowsPerPage,
...
function changeRecordsPerPage() {
    ...
    getCategory({ pagination: pagination.value, filter: filter.value });
}
...
const addCategory = e => {
    form.post(route('admin.rate-category.store'), {
        onStart: () => {
...
const deleteCategoryDialog = (item) => {
