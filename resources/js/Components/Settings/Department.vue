...
<template>
    <section class="flex-grow space-y-8 mr-5">
        ...
        <div class="border border-gray-200 rounded-xl p-6 max-w-4xl" aria-label="Add Room Type">
            <h3 class="font-bold text-lg mb-6">Add Department</h3>
        ...
        <section class="max-w-4xl space-y-4">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <h3 class="font-bold text-lg mb-3 sm:mb-0">List of Departments</h3>
            ...
            <table class="w-full border-collapse text-sm relative">
                ...
                <tr class="bg-gray-700 text-white text-left">
                    ...
                    <th class="py-2 px-3 font-semibold w-[40%]">Department</th>
                    ...
                    <td class="py-2 px-3 text-center relative">
                        <div class="inline-block relative">
                            ...
                            >
                                ...
                                <button
                                    @click="deleteDepartmentDialog(item)"
    ...
    <div v-if="showDeleteDialog"
        ...
        <div class="bg-white rounded-xl max-w-sm w-full p-6">
            <h2 class="font-semibold text-lg mb-4">Delete Department</h2>
            <p class="mb-8 text-base font-normal text-gray-900">I am sure, that i want to delete this Department</p>
...
const selectedDepartment = ref({}) // optional, used for display if needed
...
function handleSearch(val) {
    getDepartment({ pagination: pagination.value, filter: val });
}
function clearSearch() {
    filter.value = ''; // Clear the search value
    // Reset the pagination if needed (e.g., set page to 1 after clearing search)
    pagination.value.page = 1;
    getDepartment({ pagination: pagination.value, filter: '' }); // Clear the filter in the API request
}
function getDepartment(props) {
    const { page, rowsPerPage, sortBy, descending } = props.pagination;
    const filter = props.filter;

    axios.get(route('admin.department.index'), {
        params: {
            filter,
            page,
            rowsPerPage,
...
function changeRecordsPerPage() {
    ...
    getDepartment({ pagination: pagination.value, filter: filter.value });
}
...
const updateDepartment =(item) => {
    editForm.put(route('admin.department.update', item), {
        onStart: () => {
            showEditDialog.value = false;
        },
        onFinish: () => {
...
const deleteDepartmentDialog = (item) => {
...
const deleteDepartment = (item) => {
    ...
    router.delete(route('admin.department.destroy', item), {
        onSuccess: () => {
            getDepartment({ pagination: pagination.value, filter: filter.value });
