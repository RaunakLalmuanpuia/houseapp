...
const fetchStats = async () => {
    try {
        const response = await axios.get(route('statistics.status')) // adjust URL if needed
...
<template>
    <section class="flex flex-wrap gap-4 mb-4 text-[12px] font-semibold">
        ...
        <div class="w-[190px] h-[78px] bg-[#E8E5FF] text-[#150D72] rounded-md px-3 py-2 flex flex-col justify-center">
            <span class="text-[15px] font-bold leading-none">{{ stats.forwarded }}</span>
            <span class="font-normal">Forwarded</span>
        </div>
