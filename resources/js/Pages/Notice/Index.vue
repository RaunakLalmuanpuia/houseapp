...
const props = defineProps(['notice','search', 'perPage']);
...
const selectedNotice = ref({}) // optional, used for display if needed
...
const handleEdit = (item) => {
    ...
    editForm.document = item.document
...
const handleSearch = () => {
    router.get(route('admin.notice.index'), {
        search: state.search,
...
const addNotice = e => {
    form.post(route('admin.notice.store'), {
        onStart: () => {
            form.reset();
            openDropdownId.value = null
        },
...
const updateNotice = (item) => {
    ...
    formData.append('_method', 'put') // Important for PUT requests
    ...
    router.post(route('admin.notice.update', item), formData, {
        preserveScroll: true,
        onStart: () => {
...
const deleteNotice = (item) => {
    ...
    router.delete(route('admin.notice.destroy', item), {
        onSuccess: () => {
    ...
    <div class="bg-white text-black">
        <div class="ml-6">
            <header class="flex justify-between items-center mb-6">
                <h1 class="font-semibold text-base leading-5">Upload Notices</h1>
            ...
            <section class="flex flex-col md:flex-row gap-6">
                ...
                <aside class="bg-[#E8F2FD] rounded-lg p-6 w-full max-w-[320px] max-h-[500px]" >
                    <h2 class="font-semibold text-lg leading-6 mb-1">Upload File</h2>
                    <p class="text-xs leading-4 mb-6">Add new file for download</p>
                    <div class="space-y-5">
                        <div>
                            <label for="title" class="block text-xs font-normal mb-1">Title/Header</label>
                            <input
                                v-model="form.title"
                                id="title"
                                type="text"
                        ...
                        <div>
                            <label
                                ...
                                >Upload Document/Office Order</label>
                            ...
                            <div class="relative">
                                <input
                                    ...
                                    class="file:hidden bg-white w-full rounded border border-gray-300 px-3 py-2 text-xs placeholder:text-gray-400 focus:outline-none focus:ring-1 focus:ring-blue-500"
                                />
                                <span
                                    class="absolute inset-y-0 right-3 flex items-center text-gray-600 pointer-events-none"
                                    aria-hidden="true"
                                >
                                  <svg width="15" height="16" viewBox="0 0 15 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M13.4305 8.37273L8.82978 12.9735C6.5104 15.5155 4.3634 15.8222 2.38876 13.8936C0.413516 11.9644 0.720231 9.81736 3.30891 7.45258L7.90964 2.85185C9.67509 0.936722 11.2087 0.630007 12.5104 1.93171C14.4629 3.88426 13.5041 4.76576 11.5902 6.53244L6.98949 11.1332C6.11965 12.1036 5.35286 12.257 4.68913 11.5932C3.69353 10.5976 4.17845 10.1624 5.1492 9.29287L9.74993 4.69214" stroke="black" stroke-linecap="round" stroke-linejoin="round"/>
                                  </svg>
                                </span>
                            </div>
                        </div>
                        ...
                        <div>
                            <label for="status" class="block text-xs font-normal mb-1">Status</label>
                            <select
                                v-model="form.status"
                                id="status"
                ...
                <section class="flex-1 overflow-x-auto">
                    <table class="w-full border-collapse text-xs leading-5">
                        ...
                        <tr class="bg-gray-800 text-white text-left">
                            ...
                            <th class="py-2 px-3 font-semibold w-[20%]">TITLE/HEADER</th>
                            <th class="py-2 px-3 font-semibold w-[10%]">DATE & TIME</th>
                            <th class="py-2 px-3 font-semibold w-[12%]">STATUS</th>
                            <th class="py-2 px-3 font-semibold w-[12%]">VIEW PDF</th>
                            <th class="py-2 px-3 font-semibold w-[12%]">ACTION</th>
                        </tr>
                        </thead>
                        <tbody>
                        <tr v-for="(item, index) in notice.data"
                            class="border-b border-gray-100">
                            <td class="py-4 px-3">{{ index+1 }}</td>
                            <td class="py-4 px-3">{{item.title}}</td>

                            <td class="py-4 px-3">25/03/2025 10:30 am</td>
                          ...
                          item.status === 'Draft' ? 'bg-blue-200 text-blue-600' :
                          item.status === 'Published' ? 'bg-green-100 text-green-800' :
                          item.status === 'Archived' ? 'bg-red-200 text-red-700' : ''
                        ]"
                             >
                            {{ item.status }}
                          </span>
                    ...
                    >
                        ...
                        <button
                            v-for="(link, index) in notice.links"
                            :key="index"
                            :disabled="!link.url"
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input
                            ...
                            id="upload"
                            name="upload"
                            @change="e => editForm.document = e.target.files[0]"
                            accept=".pdf,.doc,.docx,.jpg,.png"
                            class="file:hidden bg-white w-full rounded border border-gray-300 px-3 py-2 text-xs placeholder:text-gray-400 focus:outline-none focus:ring-1 focus:ring-blue-500"
                        />
                        <span
                ...
                <div>
                    ...
                    <select
                        ...
                        id="status"
                        name="status"
                        class="block w-full rounded-md border border-gray-300 px-3 py-2 placeholder-gray-400 text-gray-900 text-sm focus:outline-none focus:ring-1 focus:ring-blue-600 focus:border-blue-600"
                    >
                        <option>Published</option>
                        <option>Draft</option>
                        <option>Archived</option>
                ...
                <div class="flex justify-start space-x-6 pt-2">
                    ...
                    <button
                        @click="updateNotice(selectedNotice)"
                        type="submit"
