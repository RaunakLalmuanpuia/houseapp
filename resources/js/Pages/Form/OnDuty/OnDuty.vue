    ...
    <div class="min-h-screen flex flex-col">
        ...
        <div class="flex-grow flex flex-col items-center w-[400px] mx-auto p-3">
            ...
            <div class="max-w-md w-full rounded-xl border border-gray-200 overflow-hidden">
                <header class="bg-[#E8E5FF] p-6 rounded-t-xl">
                    <h1 class="text-[#150D72] font-extrabold text-xl leading-6">ON DUTY</h1>
                    <p class="text-[#150D72] font-semibold text-sm leading-5 mt-1">Officials travelling for work</p>
                </header>
                <div class="bg-white p-6 space-y-6">
                    ...
                    <div v-if="application.on_duty_details && application.on_duty_details.length > 0">
                    ...
                    <div v-for="(on_duty, index) in application.on_duty_details" :key="index">
                        ...
                        <div>
                            ...
                            <span v-if="errors[`on_duty_details.${index}.gender`]" class="text-red-500 text-sm">
                                  {{ errors[`on_duty_details.${index}.gender`] }}
                            </span>
                        </div>
                    ...
                    <div class="flex justify-between mt-4">
                        <button
                            @click.prevent="application.addOnDuty()"
                            type="button"
...
function next() {
    ...
    router.visit(route('apply.on-duty.step-three'))
}
...
function validateForm() {
    ...
    if (!application.department_approval) {
        errors.value['department_approval'] = 'Department approval file is required.'
    }

    // on_duty_details validation
    if (application.on_duty_details?.length) {
        application.on_duty_details.forEach((item, index) => {
            if (!item.name) {
                errors.value[`on_duty_details.${index}.name`] = 'Name is required.'
            }
            if (!item.gender || !allowedGenders.includes(item.gender)) {
                errors.value[`on_duty_details.${index}.gender`] = 'Valid gender is required.'
            }
            if (!item.designation) {
                errors.value[`on_duty_details.${index}.designation`] = 'Designation is required.'
            }
            if (!item.department) {
                errors.value[`on_duty_details.${index}.department`] = 'Department is required.'
            }
            if (!item.contact) {
